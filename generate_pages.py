...
for filename, data in pages.items():
    with open(filename, 'w') as f:
        h1_html = data['h1']
        if not h1_html:
            content_html = template.replace('<div class="page-header">\n            <h1 class="animate-fade-up">{h1}</h1>\n        </div>', '')
            content_html = content_html.format(title=data['title'], content=data['content'])
            f.write(content_html)
        else:
            f.write(template.format(title=data['title'], h1=data['h1'], content=data['content']))
